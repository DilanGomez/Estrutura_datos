...
# Se reutiliza el MISMO error de ListaArreglo: el contrato exige un único tipo
# de error para ambas listas (si cada módulo definiera el suyo, serían clases
# distintas aunque se llamen igual, y `pytest.raises` no las reconocería).
...
class ListaEnlazada:
    ...
    def _eliminar_no_primero(self, posicion):
        ...
        anterior.siguiente = objetivo.siguiente     # se salta el objetivo
        if objetivo is self._cola:                  # era el último
            self._cola = anterior
